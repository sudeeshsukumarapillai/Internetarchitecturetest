...
public class NoteNirvanaApplication {
	
	public static final Logger LOG = LoggerFactory.getLogger(NoteNirvanaApplication.class); 
	...
	public static void main(String[] args) {
		...
		while (!started && retryCount < maxRetries) {
			try {
				SpringApplication.run(NoteNirvanaApplication.class, args);
				started = true;
			} catch (Exception e) {
				retryCount++;
